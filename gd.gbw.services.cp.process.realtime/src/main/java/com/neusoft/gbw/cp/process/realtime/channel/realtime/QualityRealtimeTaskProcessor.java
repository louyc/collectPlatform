...
public class QualityRealtimeTaskProcessor extends SendTaskProcessor implements ITaskProcess {
	...
	private void dispacherData(CollectData data) {
		...
		case date_collect_success:
			...
			break;
		default:
			Log.warn("[实时处理服务]指标实时查询任务处理异常，，任务执行状态：" + status + "," + getLogMsg(data));
			obj = disposeError(data);
			break;
		}
//		Log.debug("[实时处理服务]指标实时查询任务处理消息发送至前台，msg=" + obj==null?null:obj.toString());
		if(obj == null) 
			Log.debug("[实时处理服务]指标实时查询任务处理消息发送至前台，msg=" + obj);
		else
			Log.debug("[实时处理服务]指标实时查询任务处理消息发送至前台，msg=" + obj.toString());
		sendTask(obj);
	}
