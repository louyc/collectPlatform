...
public class TimeUtils {
	...
	public static List<String> getTimeSepList(String start, String end, int interval) throws Exception {
		...
		System.out.println(interval+"     interval    TimeUtils");
		long sep = interval*60*1000;
		int m=0;
		if(sep==0||interval==0){
			 m=0;
			// interval=1;
		}else{
			 m = (int) ((endTime-startTime) / sep);
		}
		
		/* 添加 避免  by zero =0 异常    替换    int m = (int) ((endTime-startTime) / sep);
		 * 	int m=0;
		if(sep==0){
			 m=0;
		}else{
		...
		System.out.println(" m :  "+ m +" startTime  : "+startTime+"  endTime :  "+ endTime);
