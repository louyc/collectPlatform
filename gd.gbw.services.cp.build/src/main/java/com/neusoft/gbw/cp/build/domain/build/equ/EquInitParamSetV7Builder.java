...
import com.neusoft.gbw.cp.conver.v7.protocol.vo.other.Center;
import com.neusoft.gbw.cp.conver.v7.protocol.vo.other.EquipmentInitParamSet;
import com.neusoft.gbw.cp.conver.v7.protocol.vo.other.InitEquipment;
import com.neusoft.gbw.cp.conver.v7.protocol.vo.other.LogInfo;
import com.neusoft.gbw.cp.conver.v7.protocol.vo.other.Param;
...
public class EquInitParamSetV7Builder {
	...
	public static Object buildEquInitParamSet(MoniSetingDTO dto, TaskPriority taskPriority) throws BuildException {
		...
		//获取厂商，如果是泰顺，则不加载接收机配置
		int manufacturer_id = info.getManufacturer_id();
		if(BuildConstants.TAI_SHUN_MANUFACTURER_ID != manufacturer_id) 
	...
	private static InitEquipment getSystemInit(MoniSetingDTO dto) {
		...
		equipment.setParams(getParamsList(dto));
		return equipment;
	}
...
//			center.setType(dto.getSystemTypId());
	...
	private static List<Param> getParamsList(MoniSetingDTO mdto) {
		List<MonitorConfDTO> confList = mdto.getConfList();
...
//		if(!mdto.getMonitorId().equals("20000000657")){
...
//		if(mdto.getMonitorId().equals("20000000657")){
		paramList.add(getParam("SpectrumScanSampleLength","1000"));
//		}
